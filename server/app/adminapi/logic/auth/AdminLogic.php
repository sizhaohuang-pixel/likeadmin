...
class AdminLogic extends BaseLogic
{
     ...
     */
    public static function add(array $params, int $currentAdminId = 0)
    {
        Db::startTrans();
        try {
            // 默认将添加者设置为新管理员的上级
    ...
    public static function edit(array $params, int $currentAdminId = 0): bool
    {
        ...
        try {
            // 权限验证：只能编辑自己的下级
            if ($currentAdminId > 0 && !AdminHierarchyService::hasPermission($currentAdminId, $params['id'])) {
                throw new \Exception('您没有权限编辑该管理员');
            }
            // 基础信息
            $data = [
                'id' => $params['id'],
    ...
    public static function delete(array $params, int $currentAdminId = 0): bool
    {
        ...
        try {
            // 权限验证：只能删除自己的下级
            if ($currentAdminId > 0 && !AdminHierarchyService::hasPermission($currentAdminId, $params['id'])) {
                throw new \Exception('您没有权限删除该管理员');
            }
            ...
            // 检查是否有下级，如果有下级则不能删除
            $subordinates = AdminHierarchyService::getSubordinateIds($params['id']);
            if (!empty($subordinates)) {
                throw new \Exception("该管理员还有下级，无法删除");
            }
