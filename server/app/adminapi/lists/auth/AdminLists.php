...
class AdminLists extends BaseAdminDataLists implements ListsExtendInterface, ListsSearchInterface, ListsSortInterface,ListsExcelInterface
{
    ...
    public function queryWhere()
    {
        ...
        if (!empty($viewableAdminIds)) {
            $where[] = ['id', 'in', $viewableAdminIds];
        } else {
            // 如果没有可查看的管理员，返回空结果
            $where[] = ['id', '=', 0];
        }
        ...
        if (isset($this->params['role_id']) && $this->params['role_id'] != '') {
            ...
            if (!empty($adminIds)) {
                // 取交集：既符合角色条件，又在权限范围内
                $allowedAdminIds = array_intersect($adminIds, $viewableAdminIds);
                if (!empty($allowedAdminIds)) {
                    $where[] = ['id', 'in', $allowedAdminIds];
                } else {
                    $where[] = ['id', '=', 0];
                }
            }
    ...
    public function lists(): array
    {
        ...
        $adminLists = Admin::field($field)
            ...
            ->append(['role_id', 'dept_id', 'jobs_id', 'disable_desc', 'online_status', 'online_status_text'])
            ->select()
            ->toArray();
