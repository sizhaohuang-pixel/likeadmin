...
// 检查是否存在相同路径的页签（忽略query参数）
const getHasSamePathTabIndex = (path: string, tabList: TabItem[]) => {
...
const useTabsStore = defineStore({
    ...
    actions: {
        ...
        addTab(router: Router) {
            ...
            const hasSamePathTabIndex = getHasSamePathTabIndex(path!, this.tabList)
            ...
            // 如果存在完全相同的fullPath，直接返回
            if (hasTabIndex != -1) {
                return
            }

            // 如果存在相同路径但不同参数的页签，更新该页签而不是创建新的
            if (hasSamePathTabIndex != -1) {
                // 更新现有页签的信息
                this.tabList[hasSamePathTabIndex] = tabItem
                // 清理旧的tasMap条目
                const oldFullPath = Object.keys(this.tasMap).find(key =>
                    this.tasMap[key].path === path && key !== fullPath
                )
                if (oldFullPath) {
                    delete this.tasMap[oldFullPath]
                }
                return
            }
