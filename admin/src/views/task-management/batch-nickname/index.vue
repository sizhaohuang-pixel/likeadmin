...
<script lang="ts" setup name="batchNicknameTaskLists">
import { reactive, ref, onMounted, onActivated, watch } from 'vue'
import { useRoute } from 'vue-router'
import { usePaging } from '@/hooks/usePaging'
import { apiBatchNicknameTaskLists, apiBatchNicknameTaskCancel, apiBatchTaskProgress, apiBatchTaskStats } from '@/api/task-management'
...
const { pager, getLists, resetPage, resetParams } = usePaging({
    fetchFun: apiBatchNicknameTaskLists,
    params: queryParams
})
...
const handleCreateTask = () => {
    ...
    console.log('showCreateTask设置为:', showCreateTask.value)
}
...
const handleCancelTask = async (row: any) => {
    try {
        ...
        const res = await apiBatchNicknameTaskCancel({ id: row.id })
...
const getStats = async () => {
    try {
        const res = await apiBatchTaskStats('batch_nickname')
...
watch(() => route.fullPath, (newPath) => {
    if (newPath.includes('/task-management/batch-nickname')) {
